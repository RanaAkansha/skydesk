...
import { Bell, Search, Plane, ChevronDown, Menu, X } from 'lucide-react'
...
export default function Navbar({ onMenuToggle, menuOpen }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-200 h-16 flex items-center px-4 gap-4 shadow-sm">
      ...
      {/* Search bar */}
      <div className="flex-1 max-w-md mx-auto hidden md:flex items-center bg-slate-100 rounded-full px-4 py-2 gap-2 border border-transparent focus-within:border-blue-300 focus-within:bg-white transition-colors">
          ...
          {notifOpen && (
            <div className="absolute right-0 top-12 w-80 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden z-50">
              ...
              <div className="divide-y divide-slate-50">
                {notifications.map(n => (
                  <div key={n.id} className={`px-4 py-3 flex gap-3 hover:bg-slate-50 cursor-pointer ${n.unread ? 'bg-blue-50/50' : ''}`}>
              ...
              <div className="px-4 py-3 border-t border-slate-100">
                <button className="text-xs text-blue-600 font-semibold hover:underline w-full text-center">View all notifications</button>
              </div>
            </div>
          )}
        </div>
